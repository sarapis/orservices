...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>SDOH Code System</h2>
        <div class="nav navbar-right panel_toolbox">
          <a href="{{route('code_systems.create')}}" class="btn btn-success">New Code System</a>
      ...
      <div class="x_content">
        ...
        @endif
        <table class="table table-striped jambo_table bulk_action" id="tblcode_systems">
          <thead>
            <tr>
          ...
          <tbody>
            @foreach($codeSystems as $codeSystem)
            <tr>
              <td>{{$codeSystem->id}}</td>
              <td>{{$codeSystem->name}}</td>
              <td>{{$codeSystem->url}}</td>
              <td>{{$codeSystem->versions}}</td>
              <td>
                <a href="{{route('code_systems.edit', $codeSystem->id)}}" class="btn btn-primary btn-xs"><i
                    class="fa fa-pencil" data-toggle="tooltip" data-placement="top" title="Edit"></i></a>
                {!! Form::open(['method'=>'DELETE', 'route' => ['code_systems.destroy', $codeSystem->id],
                'style' =>
                'display:inline']) !!}
                <button type="submit" class="btn btn-danger btn-xs" data-original-title="Delete"
                  onclick="return confirm('Are you sure to delete this Code System')" data-placement="top"><i
        ...
        {{-- @if (Sentinel::getUser()->hasAccess(['code_systems.destroy']))
...
<script type="text/javascript">
  $(document).ready(function(){
        table = $('#tblcode_systems').DataTable({
          'columnDefs': [{
            'targets': 0,
            'searchable':false,
            'orderable':false,
  ...
  $("#delete-confirm").on("click", function(){
        return confirm("Are you sure to delete this domain");
    });
