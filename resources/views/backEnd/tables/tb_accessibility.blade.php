...
<div class="row">
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
      <div class="x_title">
        <h2>Accessibility for disabilities</h2>
      ...
      <div class="x_content" style="overflow: scroll;">
        ...
        <table id="example" class="display nowrap table-striped jambo_table table-bordered table-responsive" cellspacing="0" width="100%">
            ...
            <tbody>
              @foreach($accessibilities as $key => $accessibility)
                <tr id="accessibility{{$accessibility->id}}" class="{{$accessibility->flag}}">
                  @if($source_data->active == 1 )
                  <td class="text-center">{{$key}}+1</td>
                  @elseif($source_data->active == 0)
                  <td class="text-center">{{$accessibility->accessibility_recordid}}</td>
                  @endif

                  <td>
                    @if(isset($accessibility->location()->first()->location_name))
                      <span class="badge bg-purple">{{$accessibility->location()->first()->location_name}}</span>
                    @endif
                  </td>

                  <td class="text-center">{{$accessibility->accessibility}}
                  </td>

                  <td class="text-center">{{$accessibility->accessibility_details}}
                  </td>

                  <td class="text-center">
                    {{-- <button class="btn btn-block btn-primary btn-sm open_modal"  value="{{$accessibility->accessibility_recordid}}" style="width: 80px;"><i class="fa fa-fw fa-edit"></i>Edit</button> --}}
                  </td>
                </tr>
              @endforeach
            </tbody>
